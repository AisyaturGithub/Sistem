...
if  selected == "Deskripsi":
    ...
    st.write("""
        ...
        df = pd.read_csv('https://raw.githubusercontent.com/AisyaturGithub/Skripsi/main/Data_Berita_Pariwisata.csv')
        ...
        df = pd.read_csv('https://raw.githubusercontent.com/AisyaturGithub/Skripsi/main/Data_Berita_Pariwisata_POS%20Tagging.csv')
...
if selected == "Visualisasi":
    ...
    if uploaded_files is not None:
        ...
        #st.write("WordCloud image saved as 'wordcloud_image.png'")
...
#inverted_index = create_inverted_index_from_database()
            ...
            rata2precision_koreksi = np.mean(df_hasil['Precision'].astype(float))  # Convert to float if not already

            #st.info(f"Nilai Recall: {rata2recall}") 
            st.info(f"Nilai Precision: {rata2precision_koreksi}") 
    ...
    # Inisialisasi CRFTagger dilakukan di luar loop karena kita hanya perlu melakukannya sekali
                    ...
                    # Menghitung rata-rata presisi
                    st.warning(f"Nilai Presisi: {presisi1}")
                    rata2presisi_total = (rata2precision_koreksi + presisi1) / 2
                    st.success(f"Rata-rata Nilai Presisi: {rata2presisi_total}")
...
#memanggil correction spelling dari levenshtein distance
        ...
        if uploaded_file: 
                ...
                hasil_koreksi_kata.to_excel("/xampp/htdocs/RISET_DIA/tanpaPOS.xlsx")
                ...
                st.subheader("Hasil Koreksi kata pada Data test")
                hasil_koreksi_kata = pd.DataFrame(hasil_koreksi, columns = ['Hasil Koreksi Kata'])
                hasil_koreksi_kata.to_excel("/xampp/htdocs/RISET_DIA/Data_Hasil_tanpaPOS.xlsx")
                hasil_perbandingan = pd.concat([data,hasil_koreksi_kata],axis=1)
                st.dataframe(hasil_perbandingan)
...
#memanggil correction spelling dari levenshtein distance
        ...
        # Initialize the CRFTagger
                ...
                st.subheader("Hasil Koreksi kata pada Data test")
                hasil_koreksi_kata = pd.DataFrame(hasil_koreksi, columns = ['Hasil Koreksi Kata'])
                hasil_koreksi_kata.to_excel("/xampp/htdocs/RISET_DIA/POS.xlsx")
                ...
                pos_tagging_df.to_excel("/xampp/htdocs/RISET_DIA/Data_koreksi_POS.xlsx")
                st.dataframe(pos_tagging_df)
                #mendowload hasil perhitungan presisi
                hasil_precision.to_excel("/xampp/htdocs/RISET_DIA/Data_Nilai_POS.xlsx")
                st.line_chart(hasil_precision.set_index('Query')['Precision'])  # Menampilkan line chart dari nilai precision
                rata2precision = np.mean(hasil_precision['Precision'])
                st.info(f"Rata-rata Precision: {rata2precision:.2f}")
...
if selected == "Referensi":
    ...
    st.write("""<h2 style = "text-align: center;"><a href="https://ibb.co.com/8sr7ndc"><img src="https://i.ibb.co.com/XL7Cf38/dia-foto.jpg" alt="dia-foto" border="0"  width="280" height="470"><br></h2>""",unsafe_allow_html=True) 
    #st.markdown("<h1 style='font-size:15px;text-align: left; color: black;'>NAMA  : AISYATUR RADIAH</h1>", unsafe_allow_html=True)
    st.markdown("NAMA  : AISYATUR RADIAH", unsafe_allow_html=True)
    st.markdown("NIM   : 200411100116", unsafe_allow_html=True)
    st.markdown("EMAIL : aisyaturradiah2105@gmail.com", unsafe_allow_html=True)
    st.markdown("Motto : Motto is Pray hard, work hard, and I believe that hard work always pays off.", unsafe_allow_html=True)
    st.markdown("Quotes : Menurutku kehidupan adalah keberkahan yang dengan bersama iman aku percaya, yang dengan bersama Allah hidupku Tenang.", unsafe_allow_html=True)
   
    st.title (f"SIDANG SKRIPSI")
    st.subheader("""Seputar Skripsi""")
    st.markdown("<h1 style='font-size:15px;text-align: left; color: black;'>Judul Penelitian Skripsi: KOREKSI KESALAHAN EJAAN TERHADAP QUERY PENCARIAN ARTIKEL PARIWISATA BERBAHASA INDONESIA MENGGUNAKAN ALGORITMA LEVENSHTEIN DISTANCE DAN PART-OF-SPEECH (POS)TAGGING</h1>", unsafe_allow_html=True)
    st.markdown("<h1 style='font-size:15px;text-align: left; color: black;'>Dosen Pembimbing 1 : IKA OKTAVIA SUZANTI S.Kom.,M,Cs</h1>", unsafe_allow_html=True)
    st.markdown("<h1 style='font-size:15px;text-align: left; color: black;'>Dosen Pembimbing 2 : HUSNI S.Kom.,M.T.</h1>", unsafe_allow_html=True)
    
    st.subheader("""""")
    st.markdown("<h1 style='font-size:15px;text-align: left; color: black;'>Dosen Penguji 1 : Dr. FIKA HASTARITA RACHMAN, ST., M.Eng</h1>", unsafe_allow_html=True)
    st.markdown("<h1 style='font-size:15px;text-align: left; color: black;'>Dosen Penguji 2 : Dr. RIKA YUNITARINI,ST.,MT.</h1>", unsafe_allow_html=True)
    st.markdown("<h1 style='font-size:15px;text-align: left; color: black;'>Dosen Penguji 3 : FITRI DAMAYANTI, S.Kom., M.Kom.</h1>", unsafe_allow_html=True)
    
    st.title (f"REFERENSI")
    st.subheader("""Sumber Dataset""")
    st.write("""
    Sumber data di dapatkan pada penelitian sebelumnya yang diperoleh melalui website detik.com, Berikut merupakan link untuk mengakses sumber dataset.
    <a href="https://www.detik.com/search/searchall?query=pariwisata&sortby=time&page">Detik.com dataset</a>""", unsafe_allow_html=True)
    
    st.subheader("""""")
    st.write("""Sumber data di dapatkan melalui github, Berikut merupakan link untuk mengakses sumber dataset dari github.
    <a href="https://raw.githubusercontent.com/AisyaturGithub/Skripsi/main/Data_Berita_Pariwisata.csv">github dataset dataset</a>""", unsafe_allow_html=True) 
    ...
    <a href="https://drive.google.com/drive/folders/12mCwFpmJc3QejaD_QEvZPeP8AlISE98u?usp=sharing">Drive jurnal reference</a>""", unsafe_allow_html=True) 